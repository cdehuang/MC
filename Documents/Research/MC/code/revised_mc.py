...
def simulate(configfile, m_arr, sn_type, mass, recarrdatypes=None):
    ...
    for x in range(len(ztargets)):
        ...
        number_arr = SN_number.number_SN(ztargets[x], mass, configfile, mu_min=m_arr[x,1])
        just_numbers = SN_number.recarr_to_nparr(number_arr) #things are working up to here at least
        ...
        #   change to suit the format of the lightcurve THIS PART IS INCOMPLETE. For now we will assume that the data is given in a format with days in one column (the first column) and the magnitudes in another column (the second column)
            ...
            for n in np.arange(len(SN_array)):
                day = SN_array[n][5]
                mag_1 = SN_mag.SNmag(sn_type, SN_array[n][3], day, fil=filter_name, bandwidth=bandwidth) - 0.5
                mag_11 = mumag(mag_1, SN_array[n][4])
                mag_2 = SN_mag.SNmag(sn_type, SN_array[n][3], day + cadence, fil=filter_name, bandwidth=bandwidth) - 0.5
...
if __name__=="__main__":
    ...
    filter_name = glob.glob('../../filters/WFC*{}*.dat'.format(filter))[0]
    SN_array = np.recarray((0,), dtype=recarrdatypes) # the id parameters is the combination of cluster and search number that is used by SN_number.py
    ...
    for x in range(len(ztargets)):
        ...
        #   change to suit the format of the lightcurve THIS PART IS INCOMPLETE. For now we will assume that the data is given in a format with days in one column (the first column) and the magnitudes in another column (the second column)
            ...
            for n in (np.arange(len(SN_array)-1) + 1):
                day = SN_array[n][5]
                mag_1 = SN_mag.SNmag(type, SN_array[n][3], day, fil=filter_name, bandwidth=bandwidth) - 0.5
                mag_11 = mumag(mag_1, SN_array[n][4])
                mag_2 = SN_mag.SNmag(type, SN_array[n][3], day + cadence, fil=filter_name, bandwidth=bandwidth) - 0.5
                mag_22 = mumag(mag_2, SN_array[n][4])
                SN_array[n][6] = mag_11
                SN_array[n][7] = mag_22
                flux_1 = astropysics.phot.mag_to_lum(mag_11)
