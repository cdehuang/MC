...
for s in sn_types:
    ...
    for z in target_z:
        ...
        if os.path.isfile(lightcurve_fname):
            ...
            mags = lc_dat[:,1]
            days = lc_dat[:,0]
            trimmed_mags, idx = maglim.remove_shock_breakout(mags)
            mu = maglim.magfactor(trimmed_mags, lim_mag)
        else:
             print("Missing lightcurve_{}_{}_{}.dat. Calculating light curve.".format(s, z, filter))
             l_c = maglim.whalen_lightcurve(s, z, fil=filter_name, bandwidth=bandwidth)
             mu = maglim.magfactor(l_c, lim_mag)
        mag_array[s][n] = mu
        n += 1
...
#        number_array = SNn.number_SN(z, zz[1], configfile, mu_min=minmu)
